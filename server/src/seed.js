...
/**
 * 初始化预置内容
 * @param {Object} db - 数据库对象，需提供 queryOne 和 run 方法
 */
function initPresetContents(db) {
  console.log('🔄 初始化预置内容...');
  
  PRESET_CONTENTS.forEach(content => {
    const existing = db.queryOne('SELECT id FROM contents WHERE id = ?', [content.id]);
    if (!existing) {
      db.run(
      ...
      console.log(`✅ 已添加内容: ${content.title}`);
    }
  });
  
  console.log(`✅ 预置内容初始化完成，共 ${PRESET_CONTENTS.length} 条`);
}
 ...
 */
function initPresetUsers(db) {
  console.log('🔄 初始化预置用户...');
  
  PRESET_USERS.forEach(user => {
    const existing = db.queryOne('SELECT id FROM users WHERE id = ?', [user.id]);
    if (!existing) {
      // 注意：实际应用中密码需要加密存储
      db.run(
        `INSERT INTO users (id, username, email, display_name, role, created_at) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [user.id, user.username, user.email, user.display_name, user.role, user.created_at]
      );
      console.log(`✅ 已添加用户: ${user.display_name}`);
    }
  });
  
  PRESET_PROFILES.forEach(profile => {
    const existing = db.queryOne('SELECT user_id FROM user_profiles WHERE user_id = ?', [profile.user_id]);
    if (!existing) {
      db.run(
        `INSERT INTO user_profiles (user_id, learning_style, encouragement_style, preferred_language, daily_goal, created_at) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [profile.user_id, profile.learning_style, profile.encouragement_style, 
         profile.preferred_language, profile.daily_goal, profile.created_at]
      );
      console.log(`✅ 已添加用户画像: ${profile.user_id}`);
    }
  });
  
  console.log('✅ 预置用户初始化完成');
}

/**
 * 检查数据库表是否存在
 * @param {Object} db - 数据库对象
 * @returns {boolean} 是否所有必需表都存在
 */
function checkTables(db) {
  const requiredTables = [
    'users', 'user_profiles', 'contents', 'sessions', 
    'user_content_mastery', 'reviews'
  ];
  ...
  requiredTables.forEach(table => {
    try {
      const result = db.queryOne(`SELECT name FROM sqlite_master WHERE type='table' AND name='${table}'`);
      if (!result) {
  ...
  if (missingTables.length > 0) {
    console.warn(`⚠️ 缺少数据库表: ${missingTables.join(', ')}`);
    return false;
  }
...
function initDatabase(db) {
  ...
  // 检查表结构
  if (!checkTables(db)) {
    console.error('❌ 数据库表不完整，请先运行数据库迁移脚本');
    return false;
  }
  
  // 初始化数据
  initPresetContents(db);
  initPresetUsers(db);
